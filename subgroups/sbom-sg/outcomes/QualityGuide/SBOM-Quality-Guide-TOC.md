...
また、SBOMと脆弱性情報との照合によって脆弱性が検出された場合、そのSBOMが表すソフトウェアは脆弱性を悪用したサイバー攻撃を受ける可能性がある。検出された脆弱性がどの程度悪用されやすいか、悪用された場合にどのような影響があるかによってリスクの程度が異なる。適切なリスク管理には、前述した網羅的な脆弱性の検出に加え、SBOMにおけるコンポーネント間の正確で網羅的な依存関係の記述も課題となる。<br>
（脆弱性検出や影響推定の精度は課題であると共にリスク要因でもあるが、ここでいうリスク管理の対象には含まない）  

### 5.4-2. 改善策
(問題解決のための具体的な対策（例：自動チェックツール導入、記述ルールの統一）)<br>
(改善策の実施手順の概要)
...
- SBOMと脆弱性情報との正確な照合
  ...
  1. 脆弱性情報の課題（CPE名の揺れ、CVE登録の遅れ）と改善策（曖昧照合、企業やコミュニティからの早期情報入手、PURL/SWID対応）
...
### 5.4-3. 評価方法
(改善効果を示す指標（例：チェックリストの項目とその達成率）)<br>
(定量・定性評価の実施方法と評価サイクル)
