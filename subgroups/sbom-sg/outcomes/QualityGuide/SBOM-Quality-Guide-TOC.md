...
1. 脆弱性情報と精度良く照合するためのコンポーネント情報の記述<br>
   ...
   - コンポーネントの名前とバージョンを適切に記述
     - SBOMの基本的な情報であるコンポーネントの名前とバージョンを適切に記述しておくことによって、ほとんどの脆弱性情報との照合が可能になる。ただし、脆弱性情報の種類によって名前やバージョンの記述方法に若干の違いがある。例えば、ソフトウェアの識別情報にCPEという記法を用いているNVDなどの脆弱性データベースの情報と照合する場合、コンポーネントの名前に空白があれば、その空白はアンダースコア（_）に置き換える必要がある。
   - コンポーネントのCPE名を記述
     - 上記のCPEで書かれた識別子（CPE名）をSBOMのコンポーネント情報の１つとして記述する。<br>
     CPE名の記述は、NVDなどの網羅性の高い脆弱性データベースと直接照合できることが利点だが、CPE名には表記揺れや重複が多く、まだ脆弱性が発見されていないソフトウェアにはCPE名が無い点に留意が必要。
   - コンポーネントのPURLを記述
     - CPE名と同様にPURL（Package URL）をSBOMのコンポーネント情報の１つとして記述する。<br>
     CPE名に比べて表記揺れの可能性が低いなどの利点がある一方、PURLが登録されている脆弱性データベースはまだ少ない点に留意が必要。
...
<!--
  多くのSBOM生成ツールではコンポーネント間の依存関係を出力するが、その精度や網羅性はツールやソフトウェアによって様々。NTIAのSBOM最小要素の要件を満たすことを求めているガイドラインやガイダンスに適合するためには、少なくとも上記２種類の依存関係が含まれていることを確認しておく必要がある。リスク管理の一環として、依存関係を用いて脆弱性の影響範囲を判断する場合は、あらかじめ依存関係が既知のソフトウェアで、利用するSBOM生成ツールの依存関係抽出精度を評価しておく。
-->

3. 適切なリスク管理<br>
    サイバーセキュリティの基本的なリスク管理では、下記のような指標を用いた「攻撃を受ける可能性の高さ」と「影響の大きさ」の積でリスクを評価し、これをできるだけ小さい状態に維持する。<br>
   ...
   - 影響の大きさ：脆弱性のあるコンポーネント自体やこれに依存している他のコンポーネントが停止または掌握された場合の被害の程度
    ...
    新たな脆弱性が見つかった場合、評価したリスクが大きい場合は即時対応を優先し、リスクが小さい場合は事業継続性やコストとの間で優先度を判断する。

<!-- 
リスク管理は脆弱性データベースに登録されている様々な情報を利用してリスク分析を行なうが、セキュリティアドバイザリに関する記述をSBOMとは独立に記述するポリシーの下では、SBOMに対する要件は限られているため、このガイドでは詳しく述べない。
-->

##### 5.4.4. 評価方法
<!-- 改善効果を示す指標（例：チェックリストの項目とその達成率）-->
<!-- 定量・定性評価の実施方法と評価サイクル -->
上記の改善策それぞれには、次のような評価方法がある。<br>
