...
- SBOMへのコンポーネント識別情報の網羅的記述
  ...
  2. コンポーネントの名前とバージョンを識別情報とする場合：<br>
    - SBOMの各コンポーネントにCPE（Common Platform Enumaration）の記法に合わせたソフトウェアの名前とバージョンを記述する。特にコンポーネント名の間に空白がある場合の空白はアンダースコア（_）に置き換え、バージョンはSemantic Versioningの記法で少なくともマイナーバージョンまたはパッチバージョンまで記述する。<br>
なお、CPEはNVD等の脆弱性データベースでソフトウェアパッケージの識別子に利用されているものの、脆弱性情報にOSVなどを使う場合はCPEが用いられておらず、各種の脆弱性情報で使われるソフトウェア識別子の種類によっては上記の空白などの表記方法が異なる可能性に注意する必要がある。
  3. コンポーネントのCPE名を識別情報とする場合：<br>
    - 最新のCPE Dictionaryに登録されている各コンポーネントのCPE名をソフトウェア識別子として記述する。<br>
      CPE名は同一のソフトウェアパッケージに対して複数登録されている場合があるため、その全てをSBOMに記述しておくのが良い。
[CPE Dictionary] https://nvd.nist.gov/products/cpe
  4. コンポーネントのPURLを識別情報とする場合：<br>
    - ソフトウェア開発の過程でPURLまたはSWIDを生成して付与する。今後の見通しとして、SWIDがNVD等の主要な脆弱性データベースで採用される可能性は低いと見られており、PURLを優先的に検討すべきである。PURLからCPEへの変換に取り組むプロジェクトもある（https://github.com/scanoss/purl2cpe）。<br>
    脆弱性が発見された時に作成されるCPEに対して、PURLはソフトウェアが開発された時に作成できる。現在では多くの場合自動で生成可能。ただし、開発にパッケージマネージャを使っていれば。逆にパッケージマネージャを使っていない、または開発後のバイナリしかないと、PURLで必須のタイプ（パッケージマネージャ名）を書けない場合がある。<br>

- SBOMと脆弱性情報との正確な照合
  1. SBOM側の課題と改善策（上記した識別情報の網羅的記述と同じかも）
  2. 脆弱性情報側の課題と改善策
  - CPE名の揺れ → ツールによる曖昧照合, PURLの採用<br>
    hogehoge
  - CVE登録の遅れ → 企業やコミュニティからの早期情報収集

- SBOMへの依存関係の記述によるリスク管理強化
  1. 最上位コンポーネントの特定（SPDXではDESCRIBES関係の記述, CycloneDXではcomponent階層の最上位で表現）
  2. コンポーネント間の包含関係の記述（SPDXではCONTAINS関係の記述, CycloneDXではcomponent階層で表現）
  3. 
...
- ソフトウェアの識別子にPURLを採用する場合、PURLではバージョンがオプションとなっているが、これがないと脆弱性情報との一意な対応付けができないため、PURLが自動生成される場合もSBOMの全てのコンポーネントのPURLに（SBOM自体のバージョン属性やCPEにも）バージョンが記載されていることを確認する。それでも、バージョン記述の揺れによる照合ミスのリスクは残ることに留意すべき。
