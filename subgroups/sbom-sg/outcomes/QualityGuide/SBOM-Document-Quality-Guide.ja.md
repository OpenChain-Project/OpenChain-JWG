...
-->
ソフトウェアのコンポーネント単位での脆弱性管理は、SBOMを作成する主要な目的の１つである。しかし、作成されたSBOMの内容は、必ずしも脆弱性管理や脆弱性によるリスクの管理に必要な条件を満たしていない。

##### 5.4.2. 課題の詳細
<!-- 対象課題の現状と理想状態を簡潔に説明, 図示しても良い -->
SBOMを利用して効果的に脆弱性管理やセキュリティリスク管理を行なうためには、SBOMに次のような情報が適切かつ網羅的に記載されていることが求められる。
1. ソフトウェアの各コンポーネントの脆弱性有無を知るために脆弱性情報と照合できる識別情報
2. 見つかった脆弱性への対応を依頼するための当該コンポーネントの提供者情報
3. あるコンポーネントの脆弱性が他のコンポーネントや利用者に及ぼすセキュリティリスクを評価するためのコンポーネント間の依存関係やセキュリティアドバイザリの情報
...
しかしながら、実際に作成されたSBOMには、コンポーネントの識別情報が必ずしも脆弱性情報と正確に照合できず、連絡可能なコンポーネントの提供者情報が記載されていない場合や、コンポーネント間の依存関係またはセキュリティアドバイザリの情報が付属していない場合がある。
...
1. NVD[^NVD]やOSV[^OSV]などの脆弱性データベースに登録されているソフトウェアの識別情報と同一または同じ記述形式を用いて、SBOMのコンポーネントの名前やバージョン、またはPURLを記載する。
2. SBOMのコンポーネントまたはパッケージの提供者情報には、実在し公開されている組織や個人の名前、および有効なE-mailアドレスやURLなどの問合せ先情報を記載する。
3. パッケージマネージャを利用してコンポーネント間の依存関係を取得できるツールを用いてSBOMを生成する。また、見つかった脆弱性についてCSAF[^CSAF]やOpenVEX[^OVEX]などの形式で公開されているセキュリティアドバイザリがあれば、SBOMに外部参照情報として記載する（SBOMとVEXそれぞれの情報を１つのドキュメントに併記してもよい）。

[^NVD]: National Vulnerability Database, https://nvd.nist.gov
[^OSV]: A distributed vulnerability database for Open Source, https://osv.dev
[^CSAF]: Common Security Advisory Framework Version 2.0, https://www.oasis-open.org/standard/common-security-advisory-framework-version-2-0/
[^OVEX]: OpenVEX Specification, https://github.com/openvex/spec
