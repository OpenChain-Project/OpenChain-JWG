...
1. NVD[^NVD]やOSV[^OSV]などの脆弱性データベースに登録されているソフトウェアの識別情報または同じ記述形式を用いて、SBOMのコンポーネントの名前やバージョン、またはPURLを記載する。
2. SBOMのコンポーネントまたはパッケージの提供者情報には、実在し公開されている組織や個人の名前、および有効なE-mailアドレスやURLなどの問合せ先情報を記載する。
3. パッケージマネージャを利用してコンポーネント間の依存関係を取得できるツールを用いてSBOMを生成する。また、見つかった脆弱性が影響するコンポーネントやその状態を記したVEXの情報がある場合は、SBOMに外部参照情報として記載する。または、フォーマットが対応していればSBOMとVEXを同じドキュメントに記載してもよい。[^VEX]
...
2. コンポーネントの提供者情報
- SBOM生成ツールで自動的に出力された提供者情報について、その組織名や個人名、連絡先の情報が存在することを、Web検索などによって確認する。可能であれば、それらの組織や個人によって対象のコンポーネント
