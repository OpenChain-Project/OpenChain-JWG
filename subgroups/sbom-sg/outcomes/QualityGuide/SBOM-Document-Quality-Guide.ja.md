...
しかしながら、実際に作成されたSBOMには、コンポーネントの識別情報が必ずしも脆弱性情報と正確に照合できず、連絡可能なコンポーネントの提供者情報が記載されていない場合や、コンポーネント間の依存関係が付属していない場合がある。VEXなどのセキュリティアドバイザリの情報が付属しているケースは、まだわずかである。
...
3. パッケージマネージャを利用してコンポーネント間の依存関係を取得できるツールを用いてSBOMを生成する。また、見つかった脆弱性が影響するコンポーネントやその状態を記したVEXの情報がある場合は、SBOMに外部参照情報として記載する。フォーマットが対応していればSBOMとVEXを同じドキュメントに記載してもよい。[^VEX]
